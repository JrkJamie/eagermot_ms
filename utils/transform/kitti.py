...
from utils.transform.transformation import Transformation, to_homogeneous, cam_points_to_image_coordinates
...
class TransformationKitti(Transformation):
    ...
    Args:
        ...
        calib_folder (str): calibration folder 
    """
    ...
    def img_from_tracking(self, track_points, cam="image_02", frame_data=None):
        """
        return  2d coordinates of tracks

        Args:
            track_points (numpy): nx3 3D points in rectified reference camera coordinates
            camera (str): to which camera plane perform the projection

        Returns:
            nx2 2D coordinates of points in the specified camera's image coordinates
        """
        if cam == "image_02":
    ...
    def rect_from_ref_cam(self, ref_points, only_forward=False):
        """
        rectify coordinates from camera

        Args:
            ref_points: nx3 3D points in reference camera coordinates
            only_forward: whether to clip points that are behind the image plane

        Returns: 
            nx3 3D points in rectified reference camera coordinates
        """
    ...
    def ref_cam_from_lidar(self, lidar_points):
        ...
        Args:
            lidar_points (numpy): nx3 3D points in lidar coordinates
        
        Returns:
            nx3 3D points in reference camera coordinates
        """
    ...
    def rect_from_lidar(self, lidar_points, frame_data=None, only_forward=False):
        """
        rectify coordinates from lidar

        Args:
            lidar_points (numpy): nx3 3D points in lidar coordinates
            only_forward (bool): clip points that are behind the image plane
    ...
    def img_from_lidar(self, lidar_points, cam="image_02"):
        ...
        Args:
            lidar_points (numpt): nx3 3D points lidar coordinates
            camera (str): to which camera plane perform the projection

        Returns:
            nx2 2D points in the specified camera's image coordinates
        """
