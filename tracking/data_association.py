...
from utils.inputs.bbox import Bbox3d, ProjectsToCam
...
def associate_instances_to_tracks_3d_iou(detected_instances, tracks, params: Mapping):
    """
    Assigns detected_objects to tracked objects according to method defined in params

    Args:
        detected_instances (list): detected instances
        tracks (list): tracks
    ...
    return filter_matches(matched_indices, unmatched_det_ids, unmatched_track_ids, matrix_3d_sim,
...
def associate_boxes_2d(detected_bboxes_2d, tracked_bboxes_2d, iou_threshold):
    ...
    return filter_matches(matched_indices, unmatched_detection_indices, unmatched_track_indices, iou_matrix,
                          iou_threshold)
...
def perform_association_from_similarity(first_items_len, second_items_len, similarity_matrix):
    '''
    high API to associate based on similarity

    Args:
        first_items_len (int): first len
        second_items_len (int): second len
        similarity_matrix (numpy): similarity matrix
    '''
...
def match_3d_2d_detections(dets_3d: Sequence[Bbox3d], cam: str, dets_2d: Sequence[Detection2D],
    ...
    '''
    match 3d and 2d detections

    Args:
        dets_3d (bbox3d): 3d detetcions
        cam (str): camera name
        dets_2d (Detection2D): 3d detections
        fusion_iou_threshold (float): fusion iou
        classes_to_match (int): class index

    Returns:
        matched_indices, unmatched_dets_3d_ids, unmatched_dets_2d_ids
    '''
