...
def fileparts(input_path, warning=True, debug=True):
    ...
    Returns:
        ...
        the parent directory, file name without extension, extension
    """
...
def load_list_from_folder(folder_path, ext_filter=None, depth=1, recursive=False, sort=True, save_path=None,
    ...
    Args:
        folder_path (str): root to search
        ext_filter (str): a string to represent the extension of files interested
        depth (int): maximum depth of folder to search, when it's None, all levels of folders will be searched
        recursive (bool): False: only return current level True: return all levels till to the input depth

    Returns:
        full_list, num_elem
        a list of elements, number of the elements
    """
